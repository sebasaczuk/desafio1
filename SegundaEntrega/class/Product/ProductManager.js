    ...
    return new Promise(async (resolve, reject) => {
      try {
        if (
          ...
          resolve(false); // Podes cambiar esto según lo que desees hacer en caso de que el archivo no exista
        }
      } catch (error) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (id >= 0 && id < this.lista.length) {
          ...
          this.lista[id] = updatedProduct; // Reemplazo el elemento en la lista
          this.fs.setArchivo(this.lista);
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (
          ...
          this.lista = result; // actualizo lista con archivo
          const productoIndex = this.lista.findIndex(
            (producto) => producto.code === codeNum
          );
          if (productoIndex !== -1) {
            const productoEliminado = this.lista.splice(productoIndex, 1)[0];
            this.fs.setArchivo(this.lista);
            console.log(
              `Producto con código "${codeNum}" eliminado:`,
              productoEliminado
            );
            resolve(true);
          } else {
            console.log(
              `Producto con código "${codeNum}" no encontrado en la lista.`
            );
            resolve(false);
          }
        } else {
